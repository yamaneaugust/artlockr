# Terms of Service

**Last Updated: December 11, 2025**

**Effective Date: December 11, 2025**

---

## 1. Acceptance of Terms

By accessing or using ArtLockr ("Service"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the Service.

### 1.1 Eligibility

You must be at least 13 years old to use this Service. Users between 13-18 require parental consent. By using the Service, you represent that you meet these requirements.

### 1.2 Account Registration

- You must provide accurate and complete information during registration
- You are responsible for maintaining the confidentiality of your account credentials
- You are responsible for all activities under your account
- You must notify us immediately of any unauthorized use

---

## 2. Service Description

ArtLockr provides AI-powered copyright detection services for artists:

- Upload and store artwork (feature vectors only, by default)
- Detect potential copyright infringement in AI-generated content
- API gating to block organizations from unauthorized access
- Privacy-first architecture with GDPR/CCPA compliance

### 2.1 Service Availability

- The Service is provided "as is" without guarantees of availability
- We reserve the right to modify, suspend, or discontinue the Service at any time
- We will provide reasonable notice for planned outages

---

## 3. User Content and Rights

### 3.1 Your Content

- You retain all ownership rights to artwork you upload ("Your Content")
- You grant us a limited license to process Your Content solely for providing the Service
- We will NOT use Your Content for AI training without explicit consent

### 3.2 License Grant

By uploading artwork, you grant ArtLockr a limited, non-exclusive, royalty-free license to:

- Extract feature vectors from Your Content
- Store feature vectors for copyright detection
- Display Your Content in detection results (only to you)
- Process Your Content for security and abuse prevention

This license terminates when you delete Your Content.

### 3.3 Privacy-First Storage

By default, we operate in "features-only" mode:

- Original images are deleted immediately after feature extraction
- Only 2048-dimensional feature vectors are stored
- You can opt for temporary or permanent storage if needed

### 3.4 Prohibited Content

You may not upload content that:

- Infringes on third-party intellectual property rights
- Contains illegal material
- Violates privacy or publicity rights
- Contains malware or malicious code
- Violates any applicable laws or regulations

### 3.5 Content Moderation

We reserve the right to:

- Review uploaded content for compliance
- Remove content that violates these Terms
- Suspend or terminate accounts for violations
- Report illegal content to authorities

---

## 4. Copyright and Intellectual Property

### 4.1 Copyright Detection

- Detection results are provided "as is" without legal guarantees
- Similarity scores are estimates, not definitive proof of infringement
- You are responsible for verifying and pursuing copyright claims
- We are not a party to disputes between you and alleged infringers

### 4.2 API Gating and Blocking

- You may block organizations from accessing your artwork
- Blocking is effective only within our API
- We cannot prevent unauthorized access outside our Service
- You are responsible for legal action against infringers

### 4.3 DMCA Compliance

We comply with the Digital Millennium Copyright Act (DMCA):

- To report copyright infringement, contact: [dmca@artlockr.com]
- Include: identification of copyrighted work, infringing material, contact info
- We will investigate and take appropriate action

### 4.4 Our Intellectual Property

The Service, including software, algorithms, and branding, is protected by copyright and other intellectual property laws. You may not:

- Copy, modify, or reverse engineer the Service
- Use our trademarks without permission
- Create derivative works
- Access the Service through unauthorized means

---

## 5. Privacy and Data Protection

### 5.1 Privacy Policy

Our Privacy Policy (incorporated by reference) describes how we collect, use, and protect your data. Key points:

- Features-only storage by default
- GDPR/CCPA compliant data controls
- Right to access, export, and delete your data
- Cryptographic proof of ownership

### 5.2 Data Retention

- Feature vectors: Retained until you delete them
- Detection results: Retained for 2 years or until deletion
- Account data: Retained until account deletion
- Audit logs: Retained for 7 years for security/compliance

### 5.3 Data Deletion

You can request deletion of:

- Individual artworks and their features
- Detection results
- Your entire account and all associated data

Deletion is permanent and cannot be reversed.

---

## 6. Payment and Fees

### 6.1 Free Tier

The Service offers a free tier with limitations on:

- Number of artworks
- Detection frequency
- API rate limits

### 6.2 Paid Plans (if applicable)

- Payment is required for premium features
- Fees are billed in advance on a subscription basis
- Prices are subject to change with 30 days notice
- Refunds are provided in accordance with our refund policy

### 6.3 Taxes

You are responsible for all applicable taxes. Fees are exclusive of taxes unless stated otherwise.

---

## 7. API Usage and Rate Limits

### 7.1 API Access

- API access is subject to rate limits
- Abuse or excessive use may result in suspension
- We reserve the right to modify rate limits at any time

### 7.2 API Key Security

- Keep your API keys confidential
- Do not share API keys publicly
- Rotate keys regularly
- Report compromised keys immediately

### 7.3 Prohibited API Use

You may not use the API to:

- Violate these Terms
- Circumvent security measures
- Overload or attack our systems
- Scrape or harvest data
- Resell or redistribute the Service

---

## 8. Security

### 8.1 Our Security Measures

We implement industry-standard security measures:

- Encryption in transit and at rest
- IP reputation and rate limiting
- Behavioral anomaly detection
- Regular security audits

### 8.2 Your Responsibilities

- Use strong, unique passwords
- Enable two-factor authentication if available
- Keep your devices and software secure
- Report security vulnerabilities responsibly

### 8.3 Breach Notification

In the event of a data breach, we will:

- Investigate promptly
- Notify affected users within 72 hours
- Report to relevant authorities as required
- Take corrective action

---

## 9. Acceptable Use

You agree not to:

- Use the Service for illegal purposes
- Harass, abuse, or harm others
- Upload malicious content or malware
- Attempt to gain unauthorized access
- Interfere with the Service's operation
- Violate any laws or regulations

### 9.1 Enforcement

Violations may result in:

- Warning or account suspension
- Permanent account termination
- Legal action
- Reporting to authorities

---

## 10. Disclaimers and Limitations of Liability

### 10.1 "As Is" Service

THE SERVICE IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING:

- MERCHANTABILITY
- FITNESS FOR A PARTICULAR PURPOSE
- NON-INFRINGEMENT
- ACCURACY OR RELIABILITY

### 10.2 No Guarantee

We do not guarantee:

- Detection of all copyright infringement
- Accuracy of similarity scores
- Uninterrupted or error-free service
- Security against all threats

### 10.3 Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW:

- WE ARE NOT LIABLE FOR INDIRECT, INCIDENTAL, SPECIAL, OR CONSEQUENTIAL DAMAGES
- OUR TOTAL LIABILITY IS LIMITED TO FEES PAID IN THE 12 MONTHS PRIOR TO THE CLAIM
- WE ARE NOT LIABLE FOR THIRD-PARTY ACTIONS OR CONTENT

### 10.4 Indemnification

You agree to indemnify and hold harmless ArtLockr from:

- Your violations of these Terms
- Your Content
- Your use of the Service
- Claims by third parties

---

## 11. Termination

### 11.1 Termination by You

You may terminate your account at any time by:

- Using the account deletion feature
- Contacting support

### 11.2 Termination by Us

We may suspend or terminate your account if:

- You violate these Terms
- Your account is inactive for 2+ years
- Required by law
- The Service is discontinued

### 11.3 Effect of Termination

Upon termination:

- Your access to the Service ends immediately
- Your data will be deleted according to our retention policy
- Provisions that should survive (liability, indemnification) remain in effect

---

## 12. Dispute Resolution

### 12.1 Governing Law

These Terms are governed by the laws of [JURISDICTION], without regard to conflict of law principles.

### 12.2 Arbitration

Disputes will be resolved through binding arbitration, except:

- Small claims court (under $10,000)
- Injunctive relief
- Intellectual property disputes

### 12.3 Class Action Waiver

You agree to resolve disputes individually, not as part of a class action.

---

## 13. Changes to Terms

### 13.1 Modifications

We may modify these Terms at any time. Changes will be effective:

- Immediately for new users
- 30 days after notification for existing users

### 13.2 Notification

We will notify you of material changes via:

- Email
- In-app notification
- Notice on our website

### 13.3 Continued Use

Continued use after changes constitute acceptance. If you disagree, you must stop using the Service.

---

## 14. Miscellaneous

### 14.1 Entire Agreement

These Terms constitute the entire agreement between you and ArtLockr.

### 14.2 Severability

If any provision is found invalid, the remaining provisions remain in effect.

### 14.3 No Waiver

Our failure to enforce any right does not constitute a waiver.

### 14.4 Assignment

You may not assign these Terms. We may assign them to affiliates or successors.

### 14.5 Force Majeure

We are not liable for delays or failures due to circumstances beyond our control.

---

## 15. Contact Information

For questions about these Terms:

- **Email:** legal@artlockr.com
- **Address:** [Company Address]
- **DMCA Agent:** dmca@artlockr.com

---

## 16. Age-Specific Provisions

### 16.1 Users Under 18

If you are under 18:

- You must have parental consent
- Your parent/guardian agrees to these Terms
- We may request verification of parental consent

### 16.2 Users Under 13

If you are under 13:

- You may not create an account without verified parental consent
- We comply with COPPA (Children's Online Privacy Protection Act)
- We do not knowingly collect data from children under 13 without consent

---

## 17. International Users

### 17.1 Data Transfers

If you are outside [PRIMARY JURISDICTION]:

- Your data may be transferred to our servers
- By using the Service, you consent to data transfers
- We comply with applicable data protection frameworks

### 17.2 Compliance

You are responsible for complying with local laws regarding:

- Use of the Service
- Data protection and privacy
- Copyright and intellectual property

---

## Acknowledgment

BY USING THE SERVICE, YOU ACKNOWLEDGE THAT YOU HAVE READ, UNDERSTOOD, AND AGREE TO BE BOUND BY THESE TERMS OF SERVICE.

---

**Version:** 1.0
**Last Updated:** December 11, 2025
