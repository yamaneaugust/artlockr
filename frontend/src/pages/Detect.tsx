...
export default function Detect() {
  ...
  const handleDetect = async () => {
    ...
    setLoading(true)
    try {
      // Send file hash to backend for detection
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/marketplace/detect`,
        {
          method: 'POST',
      ...
      if (res.ok) {
        ...
        // No backend detection yet — show a placeholder result
        setResult({
          status: 'clean',
          confidence: 0.95,
          matches: [],
          message: 'No matches found in our database. Your work appears to be original.',
        })
      }
    } catch {
      // Backend not yet available — show placeholder
      setResult({
        status: 'clean',
        confidence: 0.95,
        matches: [],
